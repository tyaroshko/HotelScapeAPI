...
@router.get("/rooms")
async def get_all_rooms(skip: int = 0, limit: int = 0, db: Session = Depends(get_db)):
...
async def create_new_room_type(room_type: RoomTypeCreate, db: Session = Depends(get_db)):
    return crud.create_room_type(db=db, room_type=room_type)

@router.get("/room_types/{room_type_id}")
async def get_room_type_by_id(room_type_id: int, db: Session = Depends(get_db)):
...
async def get_all_room_types(skip: int = 0, limit: int = 0, db: Session = Depends(get_db)):
